<template>
    <div>
        ...
        <div class="bookStoreMain">
            ...
            <el-card class="box-card bookCard">
                <div slot="header" class="clearfix">
                    <span>精选图书</span>
                    <el-button style="float: right; padding: 3px 0" type="text" @click="exameVisible = true">添加图书</el-button>
                </div>
                <el-row :gutter="20">
                    <el-col v-for="(item, index) in booksData" :key="index" :span="6">
                        <div class="bookStore_main_item">
                            <img  @click="toDetail(item.id)" :src="item.pic" alt="" onerror="this.src='/static/bookStore/bookDeafult.jpg'">
                            <div  @click="toDetail(item.id)" class="bookStore_main_item_title">
                                {{item.name}}
                            </div>
...
<style>
    ...
    .bookStore_main_item_title:hover {
        color: #999;
        transition: .3s;
    }
