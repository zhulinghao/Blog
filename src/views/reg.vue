...
export default {
  data() {
    const pat = new RegExp("[^a-zA-Z0-9_\u4e00-\u9fa5]", "i"); //   字母数字汉字
    ...
    return {
      ...
      rules: {
        ...
        checkPass: [{ validator: checkPass, trigger: "change" }],
      ...
      loginRules: {
        ...
        yzm: [{ validator: yzm, trigger: "change" }]
      }
    };
  },
  methods: {
    ...
    login(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          axios.post("/api/login", data).then(req => {
