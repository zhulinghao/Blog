<template>
    <div>
        <el-card class="box-card" v-for="(n,index) in articles" :key="n.id" style="padding-bottom: 10px">
                ...
                <div class="title">
                    <router-link :to="{ name:'articleDetail', params: {aid: n.id,upic: n.upic,username: n.username,createdAt: n.createdAt,uid: loginStatic.uid || 'none'} }"><b>{{n.title}}</b></router-link>
                </div>
                <div>
                ...
                <div>
                    <el-row>
                        <el-badge :value="n.commentTimes" class="comment-btn">
                            <el-button :disabled="!loginStatic.isLogin" size="small" @click="commentShow(n.id,n.upic,n.username,n.createdAt,loginStatic.uid || 'none')">
                                评论
                            </el-button>
                        </el-badge>
                        <el-badge class="item">
                            <el-button :disabled="!loginStatic.isLogin" size="small" @click="addFA(n.id,n.upic,n.username,n.createdAt,loginStatic.uid || 'none')">
                                收藏
                            </el-button>
                        </el-badge>
                    </el-row>          
    ...
    export default {
      ...
      methods: {
        commentShow(aid,upic,username,createdAt) {
            this.$router.push({ name:'articleDetail', params: {aid: aid,upic: upic,username: username,createdAt: createdAt,uid: this.loginStatic.uid || 'none'} })
        },
        show_all(index) {
